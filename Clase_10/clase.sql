...
```sql CREATE USER 'juan'@'localhost' IDENTIFIED BY 'clave123'; ```
...
```sql REVOKE ALL PRIVILEGES ON *.* FROM 'juan'@'localhost'; ```
...
```sql GRANT 'nombre_rol' TO 'nombre_usuario'@'localhost'; ```

**Ejemplo:** 

```sql GRANT 'administrador' TO 'juan'@'localhost'; ```
...
GRANT
EXECUTE ON PROCEDURE escuela.get_student_names TO 'user1' @'localhost';
...
GRANT
EXECUTE ON FUNCTION escuela.get_passing_grades TO 'user2' @'localhost';

SELECT
    nombre,
    apellido,
    get_passing_grades (nota_final) AS passing_grade
FROM alumno;
...
* * Donde se suele aplicar estos comandos * * Siempre que se encuentra una acción del tipo DML
